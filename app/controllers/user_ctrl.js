...
app.controller("UserModalInstanceCtrl", function($scope, $rootScope,$http,$modal,$modalInstance,scope,$location,loadData) {
...
app.controller("UserGroupModalCtrl", function($scope, $rootScope,$http,$modal,$modalInstance,scope,$location,loadData) {
	...
	function init(){
		...
		var defaultper= [{'name':'None','id':0},{'name':'View','id':1},{'name':'View&Entry','id':2},{'name':'View&Entry&Edit','id':3}];
	    var allowper=[{'name':'None','id':0},{'name':'Allow','id':1}];
	  
	    $scope.pcar=defaultper;
	    $scope.pcustomer=defaultper;
	    ...
	    if(scope.useredit==true){
			...
			for(pr in pArr){
				p=pArr[pr].split("_");
				if(p[0]=='CUSTOMER')
					$scope.pcustOne=$scope.pcustomer[parseInt(p[1])];
				else if(p[0]=='CAR')
					$scope.pcarOne=$scope.pcar[parseInt(p[1])];
			}
	    }
	    else
	    {
		    ...
		    $scope.pcustOne=$scope.pcustomer[0];
   	        $scope.pcarOne=$scope.pcar[0];
	    }	
	}
	...
	$scope.saveuser=function(){
    	...
    	record.permission="CUSTOMER_"+$scope.pcustOne.id;
    	record.permission+=",CAR_"+$scope.pcarOne.id;
